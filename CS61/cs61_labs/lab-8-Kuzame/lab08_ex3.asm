...
; R1 = address of data
; R2 = where the number will be operated
; R3 = Iterate to 0000 0000 0010 0000 (get the 1): 1000 0000 0000 0000
; R4 = the value negative value of x20
...
Conversion
	AND R3, R3, #0 ; in case it's not 0 already
	ADD R3, R3, 10
	LDR R2, R1, #0
	IsItLower
		ADD R2, R2, R2
		...
		BRp IsItLower
	ADD R2, R2, #0
	BRn ItIsLower	; means it'll look like 1### ##00 0000 0000
	BRzp IgnoreUpper; means it'll look like 0### ##00 0000 0000
	ItIsLower
		LDR R2, R1, #0
		ADD R2, R2, R4 ; subtract the lower case by x20
		STR R2, R1, #0
	IgnoreUpper		; does nothing and move/iterate to the next character
	ADD R1, R1, #1
	ADD R5, R5, #-1
	BRp Conversion
	BRnz ExitConversion
